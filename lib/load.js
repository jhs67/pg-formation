...
function functionName(stmt) {
	...
	if (!v.parse_tree || v.parse_tree.length !== 1 || !v.parse_tree[0].CreateFunctionStmt)
		throw new Error("expected create function statement");
	return v.parse_tree[0].CreateFunctionStmt.funcname.map(name => format("%I", name.String.str)).join('.');
}
