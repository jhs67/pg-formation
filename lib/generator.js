...
class Generator {
	...
	typeDep(n) {
		...
		let p = c.parse_tree[0].CreateStmt.tableElts[0].ColumnDef.typeName;
	...
	columnDef(name, def, deps) {
		...
		if (def.references) {
			r.push(format("REFERENCES %I", def.references));
			deps.push(`table.${def.references}`);
		}
	...
	alterTable(name, from, to) {
		...
		for (let k of columndiff.change) {
			...
			if (t.references && t.references != f.references) {
				c.push(p + format("ADD CONSTRAINT %I FOREIGN KEY(%I) REFERENCES %I",
					`${name}_${t.references}_key`, t.references, t.references));
				deps.push(`table${t.references}`);
			}
	...
	alterType(name, from, to, src) {
		...
		Object.keys(src.functions).forEach(fn => {
			...
			let m = pf[0].CreateFunctionStmt.parameters.some(
				p => this.parsedTypeDep(p.FunctionParameter.argType) == `type.${name}`);
			if (m) {
				fns.push(pf);
				c.push(this.dropFunctionCmd(pf[0].CreateFunctionStmt));
			}
		});
	...
	createFunction(name, def) {
		...
		let deps = c[0].CreateFunctionStmt.parameters.map(p => this.parsedTypeDep(p.FunctionParameter.argType));
	...
	alterFunction(name, from, to) {
		...
		let deps = [...pf[0].CreateFunctionStmt.parameters.map(p => this.parsedTypeDep(p.FunctionParameter.argType)),
			...pt[0].CreateFunctionStmt.parameters.map(p => this.parsedTypeDep(p.FunctionParameter.argType))];
		this.addCmd([ this.dropFunctionCmd(pf[0].CreateFunctionStmt), deparser.deparse(pt) ], `function.${name}`, deps);
	}

	dropFunction(name, def) {
		let pf = parseFunction(def);
		let deps = [...pf[0].CreateFunctionStmt.parameters.map(p => this.parsedTypeDep(p.FunctionParameter.argType))];
		this.addCmd(this.dropFunctionCmd(pf[0].CreateFunctionStmt), [], deps, `function.${name}`);
	}
